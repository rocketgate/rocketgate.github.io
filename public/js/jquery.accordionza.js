	...
	$.fn.accordionza = function(options) {
		...
		this.each(function() {
			...
			$slides.each(function(i) {
				...
				$(this).css('right', i * slideMinWidth + 'px').data('order', i);
			});
			...
			$slides.bind('slide', function() {
				...
				// Move all following slides to the right, and all preceding slides to the right
				$slides.filter(':gt(' + $(this).data('order') + ')').each(function() {
					$(this).stop(true).animate({right: slideMaxWidth + ($(this).data('order') - 1) * slideMinWidth + 'px'}, o.slideSpeed, o.slideEasing);
				}).end().filter(':lt(' + ($(this).data('order') + 1) + ')').each(function() {
					$(this).stop(true).animate({right: $(this).data('order') * slideMinWidth + 'px'}, o.slideSpeed, o.slideEasing);
				});
